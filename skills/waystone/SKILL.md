---
name: waystone
description: 在已绑定项目中查询、预览导入、发布、撤回或交接团队记忆；用户要求 memory init/import/recall/save/handoff/retract、明确要求查询项目历史，或在含 .waystone.json 的目录接手、继续有一定复杂度的任务以及任务告一段落时使用。
---
...
在已绑定项目中出现以下任一时机，主动把本次对话里**已经确认**的内容整理成候选记忆，询问用户是否保存：

1. 用户做出选择或拍板（如“就用 A”“定了”）；
2. 完成了用户交代的一件事（改完、写完、查明原因）；
3. 用户说出收尾的话（如“先这样”“今天到这”）。

每条候选一句话、可独立理解，标注建议的 topic、kind（decision/convention/background/handoff）和来源；已有同主题记录时说明是修订。任务未完成时附一条 handoff（进度、已验证证据、下一步）。未确认的推测、个人闲聊、密钥和临时调试信息不列入。

询问方式：支持选项提问的客户端（如 Claude Code）让用户选择“全部保存 / 挑选 / 不保存”，否则用一句文字提问。用户明确回复保存后才调用 memory_publish；未回复或转到其他话题时不保存，同一批候选不重复追问；没有值得保存的内容时不打扰用户。
